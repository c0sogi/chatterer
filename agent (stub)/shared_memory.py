...
class SharedMemoryAgent(DelegatingAgent):
        ...
        if shared_memory:
            ...
            if memory_access in [MemoryAccessLevel.READ_ONLY, MemoryAccessLevel.READ_WRITE]:
                self.register_function_as_tool(
                    self.read_from_memory, name="read_from_memory", description="Read a value from shared memory"
                )
                self.register_function_as_tool(
                    self.list_memory_keys, name="list_memory_keys", description="List all readable memory keys"
                )

            if memory_access in [MemoryAccessLevel.WRITE_ONLY, MemoryAccessLevel.READ_WRITE]:
                self.register_function_as_tool(
                    self.write_to_memory, name="write_to_memory", description="Write a value to shared memory"
                )
                self.register_function_as_tool(
                    self.delete_from_memory, name="delete_from_memory", description="Delete a value from shared memory"
                )
    ...
    def create_sub_agent(
        self, sub_agent_id: Optional[str] = None, memory_access: Optional[MemoryAccessLevel] = None
    ) -> "SharedMemoryAgent":
        """Create a sub-agent with shared memory access"""
        # Generate sub-agent ID if not provided
        actual_sub_agent_id = (
            sub_agent_id if sub_agent_id is not None else f"{self.agent_id}_sub_{len(self.sub_agents)}"
        )
    ...
    async def delegate_task(
        ...
        context = TaskContext(
            ...
            or [
                ...
                if name not in ["read_from_memory", "write_to_memory", "delete_from_memory", "list_memory_keys"]
            ],
