...
from .shared_memory import SharedMemoryAgent, SharedMemory, MemoryAccessLevel
...
async def test_tool_use_basic() -> Dict[str, Any]:
    ...
    return {
        "add_result": add_result.model_dump(),
        "mult_result": mult_result.model_dump(),
...
async def test_shared_memory_basic() -> Dict[str, Any]:
    ...
    read_only_agent = SharedMemoryAgent(
        ...
        shared_memory=shared_memory,
        memory_access=MemoryAccessLevel.READ_ONLY
    )
    ...
    write_result = await root_agent.write_to_memory(
        key="test_value",
        value=42,
        metadata={"description": "Test value"}
    )
...
async def test_context_management_basic() -> Dict[str, Any]:
    ...
    # Verify results
    assert updated_usage["current_usage"]["total_tokens"] > initial_usage["current_usage"]["total_tokens"], "Token usage did not increase"
...
async def test_metacognition_basic() -> Dict[str, Any]:
    ...
    # Create a task plan
    task_plan = await agent.create_task_plan(
        task_description="Calculate 10 + 20 and analyze the result",
        priority=2
    )
